...
export const ArabVirtualKeyboard: React.FC<ArabVirtualKeyboardProps> = ({
  ...
  // If user switched to phone's native keyboard, render a non-intrusive floating switcher dock
  if (isNative) {
    return (
      <div className="fixed bottom-2 left-1/2 -translate-x-1/2 z-50 no-print select-none" dir="rtl">
        <div className="bg-slate-900/90 backdrop-blur-md text-white px-3.5 py-1.5 rounded-full shadow-2xl border border-slate-700 flex items-center gap-3 animate-in fade-in slide-in-from-bottom-2">
          <div className="flex items-center gap-1.5 text-xs text-slate-300">
            <Smartphone className="w-3.5 h-3.5 text-emerald-400" />
            <span>لوحة مفاتيح الهاتف نشطة الآن</span>
          </div>
          <div className="w-px h-3.5 bg-slate-700" />
          <button
            onClick={toggleInputMode}
            className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition shadow-xs active:scale-95"
          >
            <KeyboardIcon className="w-3.5 h-3.5" />
            <span>التبديل إلى الكيبورد الملحق العربي ⌨️</span>
          </button>
          <button
      ...
      <div className="bg-slate-50/95 backdrop-blur-md rounded-t-xl border border-slate-300 shadow-2xl overflow-hidden ring-1 ring-slate-900/10">
        ...
        <div className="bg-slate-200/90 border-b border-slate-300 px-3 py-1 flex items-center justify-between flex-wrap gap-1">
          ...
          <div className="flex items-center gap-1.5">
            {/* Automatic Switch to Phone Keyboard */}
            <button
              onClick={toggleInputMode}
